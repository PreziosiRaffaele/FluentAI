...
const VALID_KEY_PATTERN =
  /^([A-Z0-9]|F[1-24]|Plus|Space|Tab|Backspace|Delete|Insert|Return|Enter|Up|Down|Left|Right|Home|End|PageUp|PageDown|Escape|Esc|VolumeUp|VolumeDown|VolumeMute|MediaNextTrack|MediaPreviousTrack|MediaStop|MediaPlayPause|PrintScreen)$/i;
...
async function saveMacro() {
  ...
  if (!isValidShortcut(shortcutValue)) {
    alert(
      "Invalid shortcut format. Must include at least one modifier (Command, Ctrl, Alt, Shift) and a key."
    );
    return;
  }
  ...
  await ipcRenderer.invoke("save-macros", config);
  renderTables();
...
function updateProviderSelects(field) {
  ...
  providerNames.innerHTML = providers
    .map(
...
function renderMacrosTable() {
  ...
  macrosTable.innerHTML = config.macros
    .map(
      (macro, index) => `
        <tr>
            <td>${macro.name}</td>
...
function resetAndCloseMacroModal() {
  const modal = bootstrap.Modal.getInstance(
    document.getElementById("macroModal")
  );
...
function deleteMacro(index) {
  if (confirm("Are you sure you want to delete this macro?")) {
    ...
    ipcRenderer.invoke("save-macros", config);
    renderTables();
  }
}
